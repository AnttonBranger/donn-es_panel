...
```{r}
phtest(reg_re, reg_within)
```
H0 : beta_FE = beta_RE   vs    H1 : beta_FE != beta_RE
Si beta_FE = beta_RE on choisit le modèle à effets aléatoire car les deux estimateurs sont consistents mais l'estimateur beta_RE est efficace. Sinon l'estimateur beta_RE n'est pas consistent donc on privilégie beta_FE.
On rejette l'hypothèse nulle donc on garde le modèle à effets fixes. On considère que le modèle aléatoire est biaisé. 

# Question 14

a) 
```{r}
my_model2 <- my_model <- ln_workhours ~ ln_labor_inc + educ + age
```

Effets fixe avec contrôle
```{r}
reg3_within <- plm(formula = my_model2, data = panel_final, model = "within", effect = "individual")
```

Effets aléatoires avec contrôle
```{r}
reg3_re <- plm(formula = my_model2, data = panel_final, model = "random")
```

b) 
```{r}
screenreg(list(reg_within, reg3_within, reg_re, reg3_re), 
          custom.model.names = c("Within", "Within (controle)", "RE", "RE (controle)"))
```
On a une augmentation des coefficients pour les deux modèles avec les contrôle. 

c) Test de Haussman
```{r}
phtest(reg3_re, reg3_within)
```
On rejette encore l'hypothèse nulle. On sélectionne donc le modèle à effets fixes. 
